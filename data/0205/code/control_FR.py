...
class HandControllerGui(QWidget):
	...
	def loomia_msg_cb(self, msg):
		...
		human_pressure = np.clip(self.pressure_array[7:9, 6:], 0, 1000).sum() / 10
		robot_pressure = (np.clip(self.pressure_array[:9, :2], 0, 1000).sum()
							+ np.clip(self.pressure_array[:9,3:6], 0, 1000).sum()) / 70


		actual = robot_pressure
		...
		min_grip = 0
		max_grip = max(200, robot_force)
		filtered_command = (np.clip(robot_force, min_grip, max_grip) - min_grip) / (max_grip - min_grip)

		self.grip_pressure_topic.publish(Float32(desired))
		self.robot_pressure_topic.publish(Float32(actual))
