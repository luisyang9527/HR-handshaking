...
class HandControllerGui(QWidget):
	...
	def loomia_msg_cb(self, msg):
		...
		# # grip_pressure = self.pressure_array[6:9, 6:].sum()/(self.pressure_array[6:9, 6:].shape[0]*self.pressure_array[6:9, 6:].shape[1]) # Obtained using trial and error. Need to update with loomia layout diagram from Timo
		# grip_pressure = (self.pressure_array[:9, 2:6].sum()) / (36)
		# if grip_pressure < 0 and not self.recalibration:
		...
		# 		grip_pressure = (self.pressure_array[:9, 2:6].sum()) / (36)
		# grip_pressure = (self.pressure_array[6:9, 6:].sum()) / 15
		grip_pressure = (self.pressure_array[:4,:3].sum() + self.pressure_array[:4, 4:6].sum() +
		...
		if not self.calibrated:
			self.zero_grip.append(grip_pressure)
			if len(self.zero_grip) < 20:
				return
			else:
				self.zero_grip = np.array(self.zero_grip)
				self.loomia_offset = self.zero_grip.mean()
				self.calibrated = True
		grip_pressure -= self.loomia_offset
		print(grip_pressure)
		...
		min_grip = 0
		max_grip = max(180, filtered_force_human)
		filtered_command = 5 * (np.clip(filtered_force_human, min_grip, max_grip) - min_grip) / (max_grip - min_grip)
		...
		# self.sliders[0].setValue(filtered_command)
	...
	def generate_publish_data(self):
		...
		for joint in self.joints:
			...
			# elif joint=='Cylinder left'or joint=='Pressure spring':
				...
				self.joint_pressures.append(2.5e5)
			else:
